...
exports.getRequests = async function(req, res, con, secret) { // all requests you have received
    ...
    let requests = await con.query(`SELECT * FROM ${"`Requested Book`"} WHERE Lender="${lender}" AND Status="0"`); // Replace ${response} with any other user's UUID if you want other people's notifications.
...
exports.getUserRequests = async function(req, res, con, secret) { // all requests you have sent
    ...
    let requests = await con.query(`SELECT * FROM ${"`Requested Book`"} WHERE Borrower="${uuid}"`); // Replace ${UUID} with any other user's UUID if you want other people's notifications.
